...
def get_nvd_feed():
    url = 'https://nvd.nist.gov/feeds/json/cve/1.1/nvdcve-1.1-2022.json.zip' # NVD Feed URL
    wget.download(url)
    command = 'unzip -o nvdcve-1.1-2022.json.zip' # Unzip json.gz file
    os.system(command)

def get_cpes():
    with open('cpe.txt', 'r').encode('utf-8').decode('utf-8') as v:
        cpe = v.readlines()
        return cpe
